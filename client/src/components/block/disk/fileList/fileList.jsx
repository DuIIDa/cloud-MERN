...
const FileList = () => {
    ...
    const compare = (a, b) => {
        ...
        if (elemOne < elemTwo) return sort.order
        else if (elemOne > elemTwo) return -sort.order
        return 0
    }
    ...
    if (fileView === "list") {
        return (
            <FileListContainer>
                <FileListHeader>
                    <FileListName onClick={() => sorting("name")}>
                        Название
                    </FileListName>
                    <FileListDate onClick={() => sorting("date")}>
                        Дата
                    </FileListDate>
                    <FileListSize onClick={() => sorting("size")}>
                        Размер
                    </FileListSize>
                </FileListHeader>
                ...
                {files
                    ...
                    .sort(compare)
                    .map((file) => (
                        <File key={file._id} file={file}></File>
                    ))}
                {files
                    .filter((file) => file.type !== "dir")
                    .sort(compare)
